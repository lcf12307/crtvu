...
public class GMStudentController {
   ...
   //分页显示可申请的课题
    ...
    public String list(@PathVariable int studentId,@PathVariable int page, Model model,HttpServletRequest req, HttpServletResponse resp){
        int number=10;
        req.getSession().setAttribute("studentId",studentId);
        req.getSession().setAttribute("subjectStatus",3);
        List<StudentCheckSubjectItem> allList = GMStudentService.checkNotAppliedSubject(studentId);
        ...
        if (allList.size() == 0){
            ...
            pages= (int) Math.ceil((double) GMStudentService.checkNotAppliedSubject(studentId).size()/number);
            if (pages == 1){
                pageList = allList;
            } else {
                if (page == pages){
